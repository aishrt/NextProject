import React, { useEffect, useState } from "react";
import { Metadata } from "next";
import "../../expert/expert.css";
import FinancialReport from "@/app/container/financial-report/page";
import db from "@/utils/connectDB";
import ClaimFinancialModel from "@/models/claim_financial.model";
import Case from "@/models/case.model";
import { Cases } from "@/types/Cases";
import ClaimReportModel from "@/models/cer_Report.model";
import { Report } from "@/types/Report";

export const metadata: Metadata = {
  title: "Financial Report",
  description: "Generated by create next app",
};

export default async function FinancialReportView({
  searchParams,
}: {
  searchParams: { [key: string]: string | string[] | undefined };
}) {
  let id: string | string[] | undefined | number = searchParams?.id;

  const fetchData = async (): Promise<any> => {
    await db.connectDB();

    const data = await ClaimFinancialModel.findOne({ case_id: id })
      .limit(1)
      .lean();

    const caseData: Cases | null = await Case.findById(id).lean();

    const report: Report | null = await ClaimReportModel.findOne({
      case_id: id,
    }).lean();
    return {
      data: {
        ...data,
        isFinancialReport: caseData?.isFinancialReport,
        reportStatus: report?.status,
        reportId: report?._id,
      },
    };
  };

  const data: any = await fetchData();

  return (
    <div className="main-content expert-cases case-updates">
      <FinancialReport report={data} role={"client"} />
    </div>
  );
}
