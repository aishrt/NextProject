  ...
  .superRefine(({ confirmPassword, newPassword, currentPassword }, ctx) => {
    ...
    if (newPassword == currentPassword) {
      ctx.addIssue({
        path: ["newPassword"],
        code: "custom",
        message: "The passwords should be different from old",
      });
      ctx.addIssue({
        path: ["confirmPassword"],
        code: "custom",
    ...
    try {
      ...
      if (userId) {
        const response = await axios.post(
          `/api/auth/change-password?id=${userId}&role=client`,
          values
        );
            ...
            <div className="col-12 mb-4">
              <div className="change-pass-exp relative">
                <InputField
                  ...
                  control={control}
                />
                {/* <p className="f-12 mb-0 text-end">
                  Your Alternate Password must be more than 8 characters
                </p> */}
              </div>
            </div>
            <div className="col-12 mb-4">
