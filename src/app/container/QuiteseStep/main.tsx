...
function QuitenseMain({ case_id, role }: any) {
  ...
  const getReportDetails = () => {
    setLoading(true);
    axios.get(`/api/quitense?case_id=${case_id}`).then((res: any) => {
      if (res.success) {
  ...
  useEffect(() => {
    ...
    if (reportDetails?.step == 4) {
      setActiveStep(4);
    }
    if (reportDetails?.step == 4 && reportDetails?.document) {
      setActiveStep(4);
    }
