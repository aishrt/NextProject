...
import Judicial_determination from "./Judicial_determination";
import OverTime from "./OverTime";
import ChildSupport from "./ChildSupport";
import Vexatious from "./Vexatious";
import SuddenTermination from "./SuddenTerminate";
import DismissalQuest from "./Dismissal";
import HarassmentQuest from "./Harassment";
import EvictionQuest from "./Eviction";
import UnfairQuest from "./UnfairQuest";
import NonCompititionClauseQuest from "./NonCompititionClause";
export const Questioner = ({ data }: { data: Cases | undefined }) => {
  ...
  const GetQuestions = () => {
    ...
    axios
      .get(
        `/api/category-question?case_id=${data?._id}&category=${data?.category}`
      )
      .then((res: any) => {
        setQuestions(res);
  ...
  return (
    // <Judicial_determination data={Questions} />
    <div
    ...
    >
        ...
        <div className="pi-card-inner rounded">
          <div className="p-3 border-btm-lgt">
            <span className="title-questioner f-15  semi-bold d-block">
              Q - What is your role or status in relation to the claim or issue
              at hand?
            </span>
            <span className="f-13">
              Ans -{" "}
          ...
          <div className="p-3 border-btm-lgt">
            <span className="title-questioner f-15  semi-bold d-block">
              Q - Are you seeking to build a case as an individual or as a
          ...
          <div className="p-3 border-btm-lgt">
            <span className="title-questioner f-15  semi-bold d-block">
              Q - Do you currently have legal representation for your case?
            </span>
            <span className="f-13">
            ...
            <div className="p-3 border-btm-lgt">
              ...
              <span className="f-13">
                ...
                <ul>
                  <li>
                    <span className="f-13">
                      Name: {upperFirst(data?.lawyer?.name) ?? "NA"}
                    </span>
                  </li>
                  ...
                  <li>
                    <span className="f-13">
                      Phone no.: {upperFirst(data?.lawyer?.phoneNumber) ?? "NA"}
                    </span>
                  </li>
                </ul>
              </span>
      ...
      {data?.category == "Overtime Pay Claim" && <OverTime data={Questions} />}
      {data?.category == "Child support" && <ChildSupport data={Questions} />}
      {data?.category == "Vexatious" && <Vexatious data={Questions} />}
      {data?.category == "Sudden termination" && (
        <SuddenTermination data={Questions} />
      )}
      {data?.category == "Dismissal Assessment" && (
        <DismissalQuest data={Questions} />
      )}
      {data?.category == "Harassment" && <HarassmentQuest data={Questions} />}
      {data?.category == "Eviction Compensation" && (
        <EvictionQuest data={Questions} />
      )}
      {data?.category == "Unfair Competition" && (
        <UnfairQuest data={Questions} />
      )}
      {data?.category == "Non Competition" && (
        <NonCompititionClauseQuest data={Questions} />
      )}
      {data?.category == "Judicial determination of rent" && (
        <Judicial_determination data={Questions} />
      )}
    </div>
  );
