...
function KarpaInstruction({
  ...
  useEffect(() => {
    if (data?.carpaDocument) {
      methods.setValue("carpaDocument", data?.carpaDocument);
      methods.clearErrors("carpaDocument");
    }
  }, [data]);
  ...
  const HandleSubmit = (values: any) => {
    if (!data?.carpaDocument) {
      ...
      let payload = {
        case_id,
        carpaDocument: values?.carpaDocument,
      };
  ...
  const UploadFile = (e: any) => {
    ...
    axios.post("/api/uploads", formdata).then((res: any) => {
      setLoading(false);
      methods.setValue("carpaDocument", res?.filePath);
    });
  };
  return (
              ...
              {!data?.carpaDocument ? (
                <div className="col-12 col-md-12 col-xxl-6 mt-3">
                  <input
                    ...
                    onChange={UploadFile}
                  />
                  {formState.errors["carpaDocument"] && (
                ...
                {session?.user?.role == "expert" && (
                  <>
                    ...
                    >
                      ...
                      >
                        <span className="f-16">
                          {!data?.carpaDocument && role == "expert"
                            ? "Submit"
                            : "Done"}
                        </span>
                      </LoadingButton>
                ...
                {data?.carpaDocument && (
                  <Button
                    ...
                    onClick={() => {
                      ...
                      a.href = `${BASE_URL}${data?.carpaDocument}`;
                      a.target = "__blank";
                      a.click();
                    }}
                    className="green-btn"
