...
export default function ForgotComponent() {
  ...
  const handleSubmit = async (values: FormValues) => {
    try {
      setLoading(true);
      const response = await axios.post("/api/auth/forgot-password?role=lawyer", values);
      setValues({
        email: "",
      });
      if (!response) {
  ...
  return (
    <Form<FormValues> onSubmit={handleSubmit} methods={methods}>
      <div className="row mt-3">
        ...
        <div className="mt-3 text-end">  <Link href="/auth/lawyer/login" className="bold dark f-14">Back to Login</Link></div>
      </div>
      <Snackbar {...snackProps}>
        <Alert {...alertProps} />
      </Snackbar>
