...
export async function GET(req: NextRequest) {
  try {
    ...
    const caseManagers = await Expert.find({}).countDocuments();

    const totalLawyers = await Lawyer.find({}).countDocuments();
    ...
    const activePreLitigation = await Case.find({
      status: "pending",
      caseType: "preLitigation",
    }).countDocuments();

    const activeLitigation = await Case.find({
      status: "pending",
      caseType: "litigation",
    }).countDocuments();
